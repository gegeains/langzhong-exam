...
app = Flask(__name__)
app.secret_key = "admin_secret_2026_test_abc123"  #session必需密钥
DB_NAME = "ai_study.db"
...
def init_db():
    ...
    # id,subject,grade,q_type,knowledge,q_content,answer
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        #唯一账号密码
        if uname == "yangyan" and pwd == "131452wjc":
            session["admin_login_ok"] = True
            return redirect(url_for("admin_index"))
        else:
            return '''
            <form method="post">
                <h3>账号或密码错误，无法登录</h3>
                管理员账号：<input name="username"><br>
    ...
    <form method="post">
        <h3>管理员数据后台登录</h3>
        账号：<input name="username" placeholder="请输入管理员账号"><br><br>
        密码：<input type="password" name="password" placeholder="请输入登录密码"><br><br>
...
def admin_logout():
    session.pop("admin_login_ok",None)
    return redirect(url_for("admin_login"))
